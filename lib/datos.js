...
export async function getProductos() {
  ...
  const { data, error } = await supabase
    .from("productos")
    .select(
      "id, slug, titulo, subtitulo, descripcion, autor, portada_path, precio, precio_lista, paginas, indice, destacado",
    )
    .eq("activo", true)
...
export async function getProducto(slug) {
  ...
  const { data, error } = await supabase
    .from("productos")
    .select(
      "id, slug, titulo, subtitulo, descripcion, autor, portada_path, precio, precio_lista, paginas, indice",
    )
    .eq("slug", slug)
    .eq("activo", true)
    .maybeSingle();
