...
describe("urlSitio", () => {
  ...
  // De acá salen el notification_url y las back_urls que se le mandan a
  // Mercado Pago. Una barra final en la variable de entorno produce
  // "https://cisur.vercel.app//api/webhook/mp", que rompe el aviso de pago sin
  // dar ningún síntoma: la compra se cobra y nunca se confirma.
  it("recorta la barra final", () => {
    vi.stubEnv("NEXT_PUBLIC_SITE_URL", "https://cisur.vercel.app/");
    expect(urlSitio()).toBe("https://cisur.vercel.app");
    expect(`${urlSitio()}/api/webhook/mp`).toBe(
  ...
  it("deja intacta una URL sin barra", () => {
    vi.stubEnv("NEXT_PUBLIC_SITE_URL", "https://cisur.vercel.app");
    expect(urlSitio()).toBe("https://cisur.vercel.app");
  });

  it("cae en el dominio de producción si la variable falta", () => {
    vi.stubEnv("NEXT_PUBLIC_SITE_URL", "");
    expect(urlSitio()).toBe("https://cisur.vercel.app");
  });
});

describe("enteroDePesos", () => {
  // El bug que motiva esta función: Tati escribe el precio como lo escribe
  // cualquiera en Argentina y Number() lo lee como decimal. Un material de
  // veinticinco mil pesos salía a la venta a veinticinco, el trigger lo copiaba
  // a la compra y Mercado Pago cobraba eso.
  it("acepta un entero escrito sin separadores", () => {
    expect(enteroDePesos("25000")).toBe(25000);
    expect(enteroDePesos("0")).toBe(0);
    expect(enteroDePesos(18000)).toBe(18000);
  });

  it("RECHAZA el punto de miles en vez de leerlo como decimal", () => {
    // Number("25.000") es 25. Ese era el agujero.
    expect(enteroDePesos("25.000")).toBeNull();
    expect(enteroDePesos("19.900")).toBeNull();
    expect(enteroDePesos("1.234.567")).toBeNull();
  });

  it("rechaza la coma, los signos y los espacios internos", () => {
    expect(enteroDePesos("25,000")).toBeNull();
    expect(enteroDePesos("$25000")).toBeNull();
    expect(enteroDePesos("25 000")).toBeNull();
    expect(enteroDePesos("-100")).toBeNull();
    expect(enteroDePesos("25000.5")).toBeNull();
  });

  it("tolera espacios alrededor, que es un pegado y no un error", () => {
    expect(enteroDePesos("  25000  ")).toBe(25000);
  });

  it("devuelve null ante vacío o basura", () => {
    expect(enteroDePesos("")).toBeNull();
    expect(enteroDePesos(null)).toBeNull();
    expect(enteroDePesos(undefined)).toBeNull();
    expect(enteroDePesos("gratis")).toBeNull();
    expect(enteroDePesos("1e5")).toBeNull();
  });
});
