...
export default function TextoEditable({
    ...
    return (
      <span className="block">
        <Campo
          ...
          className="w-full rounded-[2px] border border-verde bg-white px-3 py-2 font-serif text-[1.19rem] leading-relaxed text-tinta shadow-sm"
          aria-label={`Editar ${clave}`}
        />
        <span className="mt-2 flex flex-wrap items-center gap-2 text-[1.05rem]">
          <button
            type="button"
            onClick={confirmar}
            disabled={guardando}
        ...
        {error ? (
          <span className="mt-1 block text-[1.05rem] text-alerta">{error}</span>
        ) : null}
      </span>
    );
  }
  ...
  return (
    ...
    >
      {valor}
      <span aria-hidden="true" className="ml-1 align-super text-[0.95rem] text-verde-claro">
        ✎
      </span>
    </Como>
  );
